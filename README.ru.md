...
Во время активной генерации connector читает websocket progress events ComfyUI, если они доступны,
и отправляет live-проценты обратно в Filexa. Если websocket недоступен, Filexa продолжает показывать
последний известный процент этапа, а не пустую заглушку ожидания.

Registry и ComfyUI-Manager установки обновляются средствами ComfyUI/Manager, поэтому в папках без
Git connector не проверяет GitHub при старте. Для ручных Git-установок панель проверяет публичный
GitHub `plugin_info.json`; если доступна версия новее, рядом с версией появятся маркер обновления и
кнопка `Update`. Этот встроенный updater запускает только `git pull --ff-only` в папке custom node;
после скачивания обновления перезапусти ComfyUI. Он не запускает `pip install` и не устанавливает
пакеты во время работы.
...
находит первый сгенерированный media item, скачивает его через `/view` и отправляет в Filexa. Пока
результат еще не готов, он также слушает ComfyUI `/ws` progress messages для queued prompt.
